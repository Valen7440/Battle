...
def _build_list_of_strings(
    ...
    for countryball in battler.proposal:
        cb_text = (
            countryball.instance.description(short=short, include_emoji=True, bot=bot, is_trade=True)
            if not is_final
            else ""
        )
        if not cb_text:
            ...
            cb_text = f"#{instance.pk} {instance.countryball.country} (HP: {instance.health} | ATK: {instance.attack})"
            if emoji:
...
def fill_battle_embed_fields(
    ...
    if len(embed) > 6000:
        if not compact:
            return fill_battle_embed_fields(
                embed, bot, battler1, battler2, compact=True, is_admin=is_admin
            )
        else:
            embed.clear_fields()
            embed.add_field(
